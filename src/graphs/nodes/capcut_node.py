"""
CapCut Mate API 节点
输入: video_plan, audio_url, content_meta, publish_pack, review_card, material_bank
输出: draft_url, final_result
"""
import json
import logging
import requests
    ...
    try:
        ...
        if create_response.status_code != 200:
            return _build_error_result(
                ...
                f"创建草稿失败: HTTP {create_response.status_code}"
            )
        ...
        if create_data.get("code") != 0:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"创建草稿失败: {create_data.get('message', 'Unknown error')}"
            )
        ...
        if add_images_response.status_code != 200:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"添加图片失败: HTTP {add_images_response.status_code}"
            )
        ...
        if add_images_data.get("code") != 0:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"添加图片失败: {add_images_data.get('message', 'Unknown error')}"
            )
        ...
        if add_audio_response.status_code != 200:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"添加配音失败: HTTP {add_audio_response.status_code}"
            )
        ...
        if add_audio_data.get("code") != 0:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"添加配音失败: {add_audio_data.get('message', 'Unknown error')}"
            )
        ...
        if add_captions_response.status_code != 200:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"添加字幕失败: HTTP {add_captions_response.status_code}"
            )
        ...
        if add_captions_data.get("code") != 0:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"添加字幕失败: {add_captions_data.get('message', 'Unknown error')}"
            )
        ...
        if save_response.status_code != 200:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"保存草稿失败: HTTP {save_response.status_code}"
            )
        ...
        if save_data.get("code") != 0:
            return _build_error_result(
                draft_url, content_meta, publish_pack, review_card, material_bank,
                f"保存草稿失败: {save_data.get('message', 'Unknown error')}"
            )
        ...
        return {
            ...
            "cover_text": publish_pack.get("cover_text"),
        ...
        return _build_error_result(
            draft_url, content_meta, publish_pack, review_card, material_bank,
            f"CapCut API 请求失败: {str(e)}"
        )
    except Exception as e:
        logger.error(f"CapCut API error: {e}")
        return _build_error_result(
            draft_url, content_meta, publish_pack, review_card, material_bank,
            f"CapCut API 异常: {str(e)}"
        )


def _build_error_result(draft_url, content_meta, publish_pack, review_card, material_bank, error_msg):
    """构建错误结果"""
    return {
        "success": False,
        "draft_url": draft_url,
        "title": publish_pack.get("title") if publish_pack else None,
        "cover_text": publish_pack.get("cover_text") if publish_pack else None,
        "review_card": review_card,
        "material_bank": material_bank,
        "content_meta": content_meta,
        "duration": None,
